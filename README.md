...
- `--target`: instead of restoring each item to its original path, restore it
  directly under this directory using its own top-level name. For example,
  an item originally backed up from `/home/user1/Documents` gets restored to
  `<target>/Documents` rather than overwriting `/home/user1/Documents`.
  Useful for restoring to a new machine or a scratch location for
  inspection. If two items share the same top-level name, the later one in
  the manifest overwrites the earlier one under `--target` (with a warning
  logged).
